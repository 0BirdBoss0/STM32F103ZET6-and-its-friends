...
u16 dizhi        = 1;
u16 btl[5]       = {2400, 4800, 9600, 19200, 38400};
u16 bt           = 2;
u16 bt_last;

u32 regGroup[40]; // Modbus寄存器组，地址为0x00~0x04
...
void TIM2_Init()
{
    ...
    TIM_TimeBaseInitStructure.TIM_Period        = 1000;   // 自动装载值 4 （一毫秒跑一次）
...
void UartDriver()
{
    ...
    {
        flagFrame = flagFrame + 1;              // 帧接收完成标志清零
        len       = UartRead(buf, sizeof(buf)); // 将接收到的命令读到缓冲区中
        if (buf[0] == dizhi)                    // 判断地址是不是0x01
        {
            ...
            if ((buf[len - 2] == crch) && (buf[len - 1] == crcl)) // 判断CRC校验是否正确
            {
                ...
                {
                    case 0x03:                                  // 读数据
                        if ((buf[2] == 0x00) && (buf[3] <= 40)) // 寄存器地址支持0x0000~0x0005
                        {
                            ...
                            if (buf[3] <= 40) {
                                ...
                                len    = 3;
                                while (cnt--) {
                                    buf[len++] = regGroup[i] >> 8;
                                    buf[len++] = regGroup[i++];
                                    // buf[len++]=0x00;	//寄存器高字节补0
                                    // buf[len++]=regGroup[i++]/256;
                                    // buf[len++]=regGroup[i++]%256;		//低字节
                    ...
                    case 0x06:                                  // 写入单个寄存器
                        if ((buf[2] == 0x00) && (buf[3] <= 40)) // 寄存器地址支持0x0000-0x0005
                        {
                            ...
                            //					    FLASH_ProgramHalfWord(STM32_FLASH_SAVE_ADDR,regGroup[i]);//写入扇区;
                            //					    FLASH_Lock();//上锁
                            ...
                            //					    FLASH_ProgramHalfWord(STM32_FLASH_SAVE_ADDR1,regGroup[i]);//写入扇区;
                            //					    FLASH_Lock();//上锁
                            //							I_max=STM32_FLASH_ReadHalfWord( STM32_FLASH_SAVE_ADDR1);//读取扇区内容
                            //							}
                            break;
                        } else {           // 寄存器地址不被支持，返回错误码
                            buf[1] = 0x86; // 功能码最高位置1
                            buf[2] = 0x02; // 设置异常码为02-无效地址
                            len    = 3;
                            break;
...
void UartRxMonitor(u8 ms) // 串口接收监控
{
    ...
    if (USART2_RX_CNT > 0)       // 接收计数器大于零时，监控总线空闲时间
    {
        ...
        } else                             ////接收计数器未改变，即总线空闲时，累计空闲时间
        {
            ...
            {
                idletmr += ms;
                if (idletmr >= 5) // 空闲时间达到1ms时，即判定为1帧接收完毕 5-50
                {
